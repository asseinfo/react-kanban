...
describe('<Column />', () => {
  ...
  const renderCard = jest.fn((_: any, {
    title
  ...
  function mount({ children = column, ...otherProps } = {}) {
    subject = render(
      // @ts-expect-error TS(2749): 'Column' refers to a value, but is being used as a... Remove this comment to see the full error message
      <Column {...otherProps} renderColumnHeader={() => <div>Backlog</div>} renderCard={renderCard}>
        // @ts-expect-error TS(2365): Operator '<' cannot be applied to types '{ childre... Remove this comment to see the full error message
        {children}
      </Column>
    )
    return subject
