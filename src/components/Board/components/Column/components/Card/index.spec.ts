...
describe('<Card />', () => {
  ...
  function mount({ children = card, ...otherProps } = {}) {
    subject = render(
      // @ts-expect-error TS(2749) FIXME: 'Card' refers to a value, but is being used as a t... Remove this comment to see the full error message
  ...
  describe('when the card is being dragging', () => {
    ...
    beforeEach(() => {
      ...
      // @ts-expect-error TS(2304) FIXME: Cannot find name 'mount'.
      mount()
    })
