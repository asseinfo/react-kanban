...
function Card({
  ...
  return (
    ...
    <Draggable draggableId={String(children: any.id)} index={index} isDragDisabled={disableCardDrag}>
      ...
      {(provided, { isDragging }) => {
        return (
          ...
          >
            // @ts-expect-error TS(2304): Cannot find name 'div'.
            <div style={{ display: 'inline-block', whiteSpace: 'normal' }}>{renderCard(isDragging: any)}</div>
          </div>
        );
      }}
    </Draggable>
