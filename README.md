...
| [`onCardNew`](#oncardnew) (required if `allowAddCard` or `renderCardAdder` or when [`addCard`](#rendercolumnheader) is called)                     | Callback that will be called when a new card is added through the default card adder template                     | 🚫         | ✅           |
| [`onNewCardConfirm`](#onnewcardconfirm) (required if `allowAddCard` or `renderCardAdder`)                                                          | Callback that will be called when a new card is confirmed by the user through the default card adder template     | 🚫         | ✅           |
...
| function     | Description                                                |
| ------------ | ---------------------------------------------------------- |
| `addCard`  | Call this function with a new card to add the new card. This function will call the [`onNewCardConfirm`](#onnewcardconfirm) |
